...
class _CodeForgeState extends State<CodeForge> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Initialize previous value and selection
    _previousValue = _controller.text;
    _prevSelection = _controller.selection;
    ...
    _controllerListener = () {
      ...
      // Check for custom suggestion triggers on text changes
      final text = _controller.text;
      final currentSelection = _controller.selection;
      final cursorPosition = currentSelection.extentOffset.clamp(
        0,
        text.length,
      );
  ...
  /// Checks if the cursor is within a Jinja block (statement or expression).
  ///
  /// Returns true if the cursor is between matching Jinja tags (`{% ... %}` or `{{ ... }}`).
  /// Handles nested blocks and incomplete blocks (opening without closing).
